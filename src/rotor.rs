...
impl Rotor {
    ...
    pub fn forward(&self, input: char) -> char {
        ...
        let output_index = ((self.wiring[adjusted_input] as i32 + ring_setting - position + 26) % 26) as usize;
        
        trace!(
            "Rotor {} forward: {} -> {} (pos: {}, ring: {})",
            self.name, input, index_to_letter(output_index).unwrap_or('A'), 
            self.position, self.ring_setting
        );
    ...
    pub fn backward(&self, input: char) -> char {
        ...
        let ring_setting = self.ring_setting as i32;
        let adjusted_input = ((input_index + position - ring_setting + 26) % 26) as usize;
        let output_index = ((self.reverse_wiring[adjusted_input] as i32 + ring_setting - position + 26) % 26) as usize;
    ...
    pub fn step(&mut self) -> bool {
        ...
        debug!("Rotor {} gedreht: neue Position {}", self.name, self.position);
        was_at_notch
    }
    ...
    pub fn set_ring_setting(&mut self, ring_setting: usize) {
        if ring_setting < 26 {
            ...
            debug!("Rotor {} Ringstellung gesetzt auf {}", self.name, ring_setting);
        }
    }
...
pub mod rotors {
    ...
    pub fn rotor_i(ring_setting: usize, position: usize) -> Result<Rotor, String> {
        Rotor::new("EKMFLGDQVZNTOWYHXUSPAIBRCJ", 'Q', "I", ring_setting, position)
    }
    
    /// Erstellt Rotor II
    pub fn rotor_ii(ring_setting: usize, position: usize) -> Result<Rotor, String> {
        Rotor::new("AJDKSIRUXBLHWTMCQGZNPYFVOE", 'E', "II", ring_setting, position)
    }
    
    /// Erstellt Rotor III
    pub fn rotor_iii(ring_setting: usize, position: usize) -> Result<Rotor, String> {
        Rotor::new("BDFHJLCPRTXVZNYEIWGAKMUSQO", 'V', "III", ring_setting, position)
    }
    
    /// Erstellt Rotor IV
    pub fn rotor_iv(ring_setting: usize, position: usize) -> Result<Rotor, String> {
        Rotor::new("ESOVPZJAYQUIRHXLNFTGKDCMWB", 'J', "IV", ring_setting, position)
    }
    
    /// Erstellt Rotor V
    pub fn rotor_v(ring_setting: usize, position: usize) -> Result<Rotor, String> {
        Rotor::new("VZBRGITYUPSDNHLXAWMJQOFECK", 'Z', "V", ring_setting, position)
    }
    ...
    pub fn available_rotors() -> Vec<(&'static str, fn(usize, usize) -> Result<Rotor, String>)> {
        vec![
            ...
            ("III", rotor_iii as fn(usize, usize) -> Result<Rotor, String>),
